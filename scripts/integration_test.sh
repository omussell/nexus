...
#   3. For every recorded DOI, ask croid to mint (or look up) a CROID, storing
#      the result in the croid SQLite database via the same path the croid HTTP
#      server uses (POST /croid).
...
CROID_DB="$WORK_DIR/croid.sqlite3"
NAUVIS_BIN="$WORK_DIR/nauvis"
CROID_BIN="$WORK_DIR/croid"
...
echo "=== Integration Test: nauvis -> croid ==="
...
# --- Step 4: Mint a Croid for every DOI nauvis recorded, via the croid CLI ---
echo "--- Minting Croids from recorded DOIs ---"
...
# Mint one croid per DOI, deduped against whatever is already in the croid DB.
...
echo "croid CLI processed $NUM_DOIS DOIs."

# --- Step 5: Verify each DOI has a Croid in the croid database ---
echo "--- Verifying Croids in the croid database ---"
EXPECTED=$NUM_DOIS
ACTUAL=$(sqlite3 "$CROID_DB" \
  "SELECT COUNT(*) FROM cro_ids WHERE cro_type='DOI';" 2>/dev/null || echo 0)

echo "Expected $EXPECTED DOI rows; found $ACTUAL in the croid database."

if [ "$EXPECTED" -eq "$ACTUAL" ]; then
  echo "✅ Integration test passed: every DOI recorded by nauvis has a matching Croid in the croid database."
  exit 0
else
